# 教练收益统计功能 - 使用说明

## 📋 功能概述

本功能允许超级管理员查看指定教练在指定时间段内的所有课程、预订情况和收益统计。

## 🎯 核心功能

1. **按教练查询**: 选择特定教练查看其收益统计
2. **时间范围筛选**: 支持按开始日期和结束日期筛选
3. **详细统计**: 显示每个课程的详细信息、预订数量和收益
4. **汇总数据**: 自动计算课程总数、总预订数和总收益

## 📊 数据统计逻辑

### 收益计算方式

```
课程收益 = 课程费用 × 预订数量
总收益 = 所有课程收益之和
```

### 统计范围

- **包含的订单**: 只统计 `payment_status = '1'`（已支付）的订单
- **退款订单**: 不计入收益统计（退款后 payment_status = '2'）
- **未支付订单**: 不计入收益统计

## 🚀 使用方法

### 访问路径

- **URL**: `/order/coach/revenue`
- **路由**: `/order/coach/revenue`
- **权限**: 需要登录管理员账号

### 操作步骤

1. **登录系统**
   - 使用管理员账号登录（userType = '1'）

2. **进入页面**
   - 在浏览器地址栏输入 `/order/coach/revenue`
   - 或在管理端菜单中点击"教练收益统计"

3. **选择教练**
   - 在"选择教练"下拉框中选择要查询的教练
   - 支持搜索教练姓名快速定位

4. **设置时间范围（可选）**
   - 选择"开始日期"和"结束日期"
   - 不设置时间范围则查询所有历史数据

5. **点击"查询"按钮**
   - 系统显示该教练的收益统计列表
   - 顶部显示汇总数据

## 📱 页面功能说明

### 筛选区域

| 字段 | 说明 | 必填 |
|------|------|------|
| 选择教练 | 从下拉框选择教练 | ✅ 是 |
| 开始日期 | 课程开始日期的起始范围 | ❌ 否 |
| 结束日期 | 课程开始日期的结束范围 | ❌ 否 |

### 统计摘要

- **课程总数**: 该教练在指定时间范围内的课程数量
- **总预订数**: 所有课程的总预订人次
- **总收益**: 所有已支付订单的金额总和

### 收益明细表格

| 列名 | 说明 |
|------|------|
| 课程ID | 课程的唯一标识 |
| 课程名称 | 课程的具体名称 |
| 课程费用 | 单节课程的价格 |
| 开始时间 | 课程的开始时间 |
| 结束时间 | 课程的结束时间 |
| 预订数量 | 该课程的预订人数 |
| 课程收益 | 该课程的收益金额（费用×人数） |

## 🔧 技术实现

### 后端接口

#### 1. 查询教练收益列表
```
GET /order/coach/revenue
```

**请求参数**:
- `coachId` (必填): 教练ID
- `startDate` (可选): 开始日期，格式: YYYY-MM-DD
- `endDate` (可选): 结束日期，格式: YYYY-MM-DD

**响应示例**:
```json
{
  "code": 1,
  "msg": "success",
  "data": [
    {
      "courseId": 101,
      "courseName": "瑜伽基础",
      "scheduleStart": "2026-02-01T10:00:00",
      "scheduleEnd": "2026-02-01T11:30:00",
      "courseFee": 50.00,
      "bookingCount": 5,
      "totalRevenue": 250.00
    }
  ]
}
```

#### 2. 查询教练总收益
```
GET /order/coach/totalRevenue
```

**请求参数**: 同上

**响应示例**:
```json
{
  "code": 1,
  "msg": "success",
  "data": 250.00
}
```

### 数据库查询

**SQL逻辑**:
```sql
SELECT
    c.course_id AS courseId,
    c.course_name AS courseName,
    c.schedule_start AS scheduleStart,
    c.schedule_end AS scheduleEnd,
    c.course_fee AS courseFee,
    COUNT(DISTINCT b.booking_id) AS bookingCount,
    SUM(p.amount) AS totalRevenue
FROM gym_courses c
    LEFT JOIN gym_coachs co ON c.coach_id = co.coach_id
    LEFT JOIN gym_booking b ON c.course_id = b.course_id
    LEFT JOIN gym_payments p ON b.booking_id = p.booking_id
    AND p.payment_status = '1'
WHERE co.coach_id = #{coachId}
    AND DATE(c.schedule_start) >= #{startDate}
    AND DATE(c.schedule_start) <= #{endDate}
GROUP BY c.course_id, c.course_name, c.schedule_start, c.schedule_end, c.course_fee
ORDER BY c.schedule_start DESC
```

## 💡 使用场景

### 场景1: 月度教练绩效评估
- 操作：选择教练，设置当月1日到月末为时间范围
- 用途：评估教练的月度业绩和收益贡献

### 场景2: 季度收益统计
- 操作：选择教练，设置季度起止日期
- 用途：分析季度内的业务表现和收入变化

### 场景3: 年度总收益查询
- 操作：选择教练，不设置时间范围（或设置全年）
- 用途：统计年度总收益，用于年终总结和奖金计算

## ⚠️ 注意事项

1. **时间范围**: 课程时间指的是 `schedule_start`，即课程开始时间
2. **退款处理**: 退款的订单不计入收益统计
3. **未支付订单**: 支付状态为"未支付"的订单不计入统计
4. **数据实时性**: 收益数据实时计算，基于当前数据库状态

## 🔍 数据关系

```
gym_coachs (教练表)
    ↓ coach_id
gym_courses (课程表)
    ↓ course_id
gym_booking (预订表)
    ↓ booking_id
gym_payments (支付表) [仅 payment_status = '1']
```

## 📝 更新日志

- **2026-02-10**: 创建教练收益统计功能
  - 支持按教练和时间范围查询
  - 提供详细的课程收益明细
  - 自动计算汇总统计数据

## 🆘 常见问题

### Q1: 为什么某些课程没有显示收益数据？
A: 可能有以下原因：
- 该课程没有已支付的预订记录
- 课程的时间不在指定的时间范围内

### Q2: 退款后的课程如何计算收益？
A: 退款后的订单 `payment_status` 变为 '2'，不计入收益统计

### Q3: 能否同时查询多个教练的收益？
A: 当前版本不支持，一次只能查询一个教练的收益。建议分别查询后手动汇总。

### Q4: 如何按月份快速查看收益？
A: 设置开始日期为月初（如2026-02-01），结束日期为月末（如2026-02-28）

---

**开发者**: guess-you
**最后更新**: 2026-02-10
**版本**: 1.0.0
